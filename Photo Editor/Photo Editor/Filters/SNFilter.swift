...
open class SNFilter: UIImageView {
    ...
    open static func generateFilters(_ originalImage: SNFilter, filters:[String]) -> [SNFilter] {
        ...
        let frame = originalImage.frame
        let image = originalImage.image!.fixOrientation()
        for filter in filters {
            let filteredImage = SNFilter(frame: frame, withImage: image)
            filteredImage.applyFilter(filterNamed: filter)
            finalFilters.append(filteredImage)
